...
(function (root, factory) {
  ...
  // いくらでもコードを読み続ける(実測で下調べ67〜129秒、点検43〜247秒)。
  // 前後の工程が待たされるので、調べものには本作業より短い時間を割り当てる。
  // ただし実測より短くしないこと。時間切れが既定になると、点検を通っていない
  // ものを通ったものと区別できなくなる。
  const STAGES = {
    survey: {
      label: '下調べ',
    ...
    review: {
      label: '点検',
      read: true,
      // 実測でばらつく(43秒〜247秒)。コメントに「10分近くかかった」と書いて
      // おきながら5分にしていたので、時間切れが例外ではなく既定になっていた。
      timeoutMs: 8 * 60 * 1000,
